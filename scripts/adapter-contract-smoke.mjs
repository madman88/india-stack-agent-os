...
const cashflow = await readCashflowAttestation();
...
const demand = await readDemandSignals();
...
const compliance = await readComplianceAttestation();
...
const mandate = await prepareRepaymentMandate({ amount: 72000, repaymentCapDays: 45 });
...
const proof = await writeProofEvent({
  label: "Adapter contract proof",
  detail: "Proof event emitted through normalized Finternet adapter",
  status: "verified"
});
assertKeys(proof, ["id", "time", "label", "rail", "hash", "status", "detail"], "Finternet proof");
assert(proof.rail === "Finternet", "Finternet proof rail mismatch");
assert(proof.status === "verified", "Finternet proof status mismatch");

const decision = await createWorkingCapitalDecision("ravi-stores");
...
const approved = await captureApproval({ businessId: "ravi-stores", action: "approve" });
...
const rejected = await captureApproval({ businessId: "ravi-stores", action: "reject" });
